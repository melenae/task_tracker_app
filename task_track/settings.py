...
MIDDLEWARE = [
    ...
    'erp_tools.middleware.DisableCSRFForAPI',  # Отключаем CSRF для API
    'django.middleware.csrf.CsrfViewMiddleware',  # CSRF все еще нужен для админки
...
TEMPLATES = [
    {
        ...
        'DIRS': [],  # Templates удалены - используем React
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
# CSRF настройки
# API endpoints используют csrf_exempt в urls.py
# CSRF все еще активен для админки Django
CSRF_COOKIE_HTTPONLY = False  # Позволяет JavaScript читать CSRF токен
CSRF_COOKIE_SAMESITE = 'Lax'  # Для работы с cross-origin запросами
...
REST_FRAMEWORK = {
    ...
    'PAGE_SIZE': 20,
    # Отключаем CSRF для REST Framework (используем SessionAuthentication)
